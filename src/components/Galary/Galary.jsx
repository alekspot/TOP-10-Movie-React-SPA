...
const Galery = ({images}) => {
  ...
  return (
    <div className="galery" style={images.length <=3 ?{maxWidth: 420}:{}}>
      
      <Slider {...settings}>
        {images.map((img, index) => {
        return (
          <div key={index}  className="galery__slide">
            <div key={index} onClick={() => onClickImg(img)} className="galery__item" style={{backgroundImage: `url(${img})`}}></div>
          </div>
        )
        })}
